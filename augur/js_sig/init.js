...
async function main() {
  ...
  const contract6 = cfx.Contract({
    ...
    bytecode: require('./contracts/SigSimpleUniverse-bytecode.json'),
  });
  // deploy the contract, and get `contractCreated`
  const receipt6 = await contract6.constructor(receipt5.contractCreated, 200)
  .sendTransaction({ from: accountminer})
  .confirmed();
  ...
  const contractUni = cfx.Contract({
    abi: require('./contracts/SigSimpleUniverse-abi.json'),
    address: receipt6.contractCreated,
  });
...
cp ~/Documents/augur/packages/augur-core/source/contracts/out/SigSimpleUniverse.* .
